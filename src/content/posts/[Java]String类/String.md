...
category: "Java"
author: "Mem0rin"
sourceLink: "https://blog.csdn.net/2501_93882415/article/details/159252153"
draft: false
pinned: false
image: "./cover.jpg"
licenseName: "MIT"
...
- [二、字符串的比较](#二字符串的比较)
  - [== 比较字符串地址](#比较字符串地址)
  - [equals() 方法判断字符串的值是否相等](#equals-方法判断字符串的值是否相等)
  - [compareTo() 方法比较字符串大小](#compareto-方法比较字符串大小)
...
| 方法 | 功能 |
| --- | --- |
| `char charAt(int index)` | 返回指定索引位置的字符。 若索引为负数或越界，抛出 `IndexOutOfBoundsException` |
| `int indexOf(int ch)` `int indexOf(int ch, int fromIndex)` `int indexOf(String str)` `int indexOf(String str, int fromIndex)` | 返回指定字符（或字符串）第一次出现的位置。 可指定起始搜索位置。 未找到时返回 `-1` |
| `int lastIndexOf(int ch)` `int lastIndexOf(int ch, int fromIndex)` `int lastIndexOf(String str)` `int lastIndexOf(String str, int fromIndex)` | 从后向前搜索，返回指定字符（或字符串）第一次出现的位置。 可指定起始搜索位置。 未找到时返回 `-1` |
...
| 方法 | 功能 |
| --- | --- |
| `String[] split(String regex)` `String[] split(String regex, int limit)` | 将字符串按正则表达式拆分。 可指定拆分的最大组数 `limit`，若省略则全部拆分。 |
| `String substring(int beginIndex)` `String substring(int beginIndex, int endIndex)` | 截取字符串的子串。 可指定起始索引与结束索引（不含），若只传起始索引则截取到末尾。 |
...
| `String replaceAll(String regex, String replacement)` `String replaceFirst(String regex, String replacement)` | 使用正则表达式进行替换。 `replaceAll` 替换所有匹配项，`replaceFirst` 仅替换第一个匹配项。 |
